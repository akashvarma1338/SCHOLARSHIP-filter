...
@app.route('/admin/bulk-processing')
@admin_required
def admin_bulk_processing():
    """Admin bulk processing page"""
    return render_template('admin_bulk_processing.html')
...
@admin_required
def bulk_upload():
    """Handle bulk student data upload (Excel/PDF)"""
    try:
        ...
        if not allowed_file(file.filename):
            return jsonify({'success': False, 'error': 'Invalid file type. Only Excel (.xlsx, .xls) and PDF files are allowed'}), 400
        
        # Save uploaded file
        filename = secure_filename(file.filename)
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], f"bulk_{uuid.uuid4().hex}_{filename}")
        file.save(file_path)
        ...
        if filename.lower().endswith(('.xlsx', '.xls')):
            ...
            df = processor.read_pdf_file(file_path)
        else:
        ...
        return jsonify({
            ...
            'eligible_students': sum(1 for r in results if any(s['eligible'] for s in r['scholarships']))
        })
...
@app.route('/api/admin/download-results', methods=['POST'])
@admin_required
def download_results():
    """Download eligibility results as Excel or PDF"""
    try:
        data = request.get_json()
        results = data.get('results', [])
        format_type = data.get('format', 'excel')  # 'excel' or 'pdf'
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=f'.{"xlsx" if format_type == "excel" else "pdf"}') as tmp_file:
            if format_type == 'excel':
                output_path = processor.generate_excel_report(results, tmp_file.name)
                mimetype = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
                filename = f'scholarship_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
            else:
                output_path = processor.generate_pdf_report(results, tmp_file.name)
                mimetype = 'application/pdf'
                filename = f'scholarship_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
