    ...
    def check_eligibility(self, student, scholarship):
        ...
        for rule in scholarship.rules:
            ...
            if not passed:
                ...
                # Add acceptance reason for passed rules
                if rule.description:
                    acceptance_reasons.append(f"✓ {rule.description} - Satisfied")
                else:
                    acceptance_reasons.append(f"✓ {rule.field} {rule.operator} {rule.value} - Satisfied")
